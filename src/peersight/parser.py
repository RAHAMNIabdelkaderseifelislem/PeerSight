...
def parse_review_text(review_text: str) -> Optional[Dict[str, str]]:
    ...
    for header in known_headers_ordered:
        ...
        if key: parsed_data[key] = ""

    current_known_header_key = None
    start_index = 0
    if parts and parts[0].strip() == "": start_index = 1

    for i in range(start_index, len(parts)):
        part = parts[i]
        # Corrected check: Need to strip before matching header pattern
        part_stripped = part.strip()
        is_header_line = re.match(r"##\s+.*", part_stripped) is not None # Check stripped part
    ...
    for key in required_keys:
        content = parsed_data.get(key, "").strip() # Get content, default to "", strip whitespace
        if not content: # Check if content is empty after stripping
             logger.error(f"Parsing validation failed: Required section '{key}' is missing or empty.")
        ...
        if key == 'recommendation':
            ...
            if processed_recommendation in valid_recommendations:
                ...
                logger.warning(f"Parsing warning: Invalid recommendation found: '{recommendation}'. Storing raw value.")
                final_parsed_data[key] = recommendation # Store raw (still counts as present)
